...
export function PointCloudCanvas3D({
  ...
  const selectedPointcloudMapArtifact = useMemo(
    () => selectedMap?.artifacts.find((artifact) => artifact.kind === "pointcloud_map_3d") ?? null,
    [selectedMap],
  );
  const activeSavedPointcloudPath = useMemo(
    () =>
      selectedPointcloudPath ??
      selectedSnapshotArtifact?.path ??
      selectedNativeArtifact?.path ??
      selectedPointcloudMapArtifact?.path ??
      null,
    [selectedNativeArtifact, selectedPointcloudMapArtifact, selectedPointcloudPath, selectedSnapshotArtifact],
  );
