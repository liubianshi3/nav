...
POINTCLOUD_3D_ARTIFACT_KINDS = {
    "pointcloud_snapshot_3d",
    "native_pointcloud_map_3d",
    "pointcloud_map_3d",
}
...
NAVIGATION_NODES_2D: list[tuple[str, str, PatternSpec]] = [
...
NAVIGATION_NODES_3D: list[tuple[str, str, PatternSpec]] = [
    ...
    ("relocalizer", "3D PCD relocalizer", "ndt_adapter_node"),
    ("localization", "3D localization gate", "localization_gate"),
    ("goal_bridge", "3D goal bridge", "goal_bridge"),
    ("nav2_controller", "Nav2 controller", "controller_server"),
...
class StackController:
    ...
    def _start_script_command(self, mode: str, map_id: str | None = None, navigation_type: str = "3d") -> list[str]:
        if navigation_type == "2d" and mode == "navigation":
            start_real = self.start_script.with_name("start_real_stack.sh")
            if not start_real.exists():
                raise StackControlError(f"2D 启动脚本不存在: {start_real}")
            command = [str(start_real), self.config.stack.network_interface]
            command.extend(["enable_nav2_bringup:=true", "enable_control_bridge:=true"])
            if map_id:
                map_info = self.get_map(map_id)
                if map_info and map_info.map_yaml:
                    command.append(f"map:={map_info.map_yaml}")
            return command
    ...
    def start_navigation(self, map_id: str, navigation_type: str = "3d") -> dict[str, str]:
        if not self.start_script.exists():
            raise StackControlError(f"启动脚本不存在: {self.start_script}")
        ...
        if navigation_type not in ("2d", "3d"):
            raise StackControlError(f"不支持的导航类型: {navigation_type}")
        use_3d_navigation = navigation_type == "3d"
        try:
            result = self._run(
                self._start_script_command("navigation", map_info.map_id, navigation_type=navigation_type),
                env={
    ...
    def start_2d_navigation(self, map_id: str) -> dict[str, str]:
        """显式启动 2D 导航栈 (AMCL + slam_toolbox + 2D Nav2)。"""
        return self.start_navigation(map_id, navigation_type="2d")
    ...
    def _expected_nodes_for_mode(
        ...
        *,
        ...
        if mode == "navigation":
            ...
            return NAVIGATION_NODES_2D
        return MAPPING_NODES

    def _wait_for_expected_nodes(self, mode: str, *, use_3d_navigation: bool = True) -> None:
    ...
    def list_maps(self, *, include_incompatible: bool = False) -> list[SavedMapInfo]:
        ...
        for item in sorted(self.map_root.iterdir(), key=lambda path: path.name):
            ...
            map_info = SavedMapInfo(
                ...
                has_pointcloud_3d=(
                    metadata.get("representation") == "pointcloud_map_3d" or
                    any(
                        artifact.kind in POINTCLOUD_3D_ARTIFACT_KINDS
    ...
    def status(self) -> StackStatus:
        ...
        return StackStatus(
            mode=mode,
            navigation_type=runtime_state.get("navigation_type"),
            pid=self._read_pid(),
    ...
    def _infer_mode(self, processes: list[ProcessInfo]) -> str:
        has_nav = any(
            ...
            or "ndt_adapter_node" in proc.args
    ...
    def _describe_process(self, process: ProcessInfo) -> str:
        for _, label, pattern in NAVIGATION_NODES_2D + NAVIGATION_NODES_3D + MAPPING_NODES:
