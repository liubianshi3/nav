...
def create_app(config_path: str | None = None) -> FastAPI:
    ...
    @app.post("/api/stack/start-2d")
    async def start_2d_navigation_stack(request: StartNavigationRequest):
        try:
            result = await asyncio.to_thread(stack_controller.start_2d_navigation, request.map_id)
        except StackControlError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc
        return {ok: True, **result, stack: jsonable_encoder(stack_controller.status())}

    @app.post("/api/stack/switch")
    async def switch_stack(request: StackSwitchRequest):
        try:
            current = stack_controller.status()
            if current.mode not in ("stopped", None):
                await asyncio.to_thread(stack_controller.stop)
            if request.navigation_type == "2d":
                if not request.map_id:
                    raise HTTPException(status_code=400, detail="2D 导航需要指定 map_id")
                result = await asyncio.to_thread(stack_controller.start_2d_navigation, request.map_id)
            elif request.navigation_type == "3d":
                if request.map_id:
                    result = await asyncio.to_thread(stack_controller.start_navigation, request.map_id)
                else:
                    result = await asyncio.to_thread(stack_controller.start_mapping)
            else:
                raise HTTPException(status_code=400, detail=f"不支持的导航类型: {request.navigation_type}")
        except StackControlError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc
        return {ok: True, **result, stack: jsonable_encoder(stack_controller.status())}
