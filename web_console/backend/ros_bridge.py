...
class RosBridgeNode(Node):
    ...
    def set_initial_pose(self, request: InitialPoseRequest) -> dict[str, Any]:
        with self._navigation_lock:
            ...
            is_3d_map = self.config.ros.map_representation == "pointcloud_map_3d"
            
            if not uses_pose_topic_3d and not is_3d_map and not self.map_snapshot.loaded:
                raise RosBridgeError("地图尚未加载完成")

            if uses_pose_topic_3d or is_3d_map:
...
class RosRuntime:
    ...
    def start(self) -> None:
        ...
        with self.node._lock:
            ...
            self.node.health.last_error = None
        self._started = True
    ...
    def stop(self) -> None:
        ...
        try:
            if rclpy.ok():
                rclpy.shutdown()
        except Exception:  # pragma: no cover - best-effort shutdown during restart
            pass
        self._started = False
