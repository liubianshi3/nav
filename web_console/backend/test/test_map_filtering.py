...
def test_list_maps_marks_pointcloud_map_3d_artifact_as_3d(tmp_path):
    controller = _build_controller(tmp_path)
    map_dir = controller.map_root / "dlio_21534_map"
    map_dir.mkdir(parents=True, exist_ok=True)
    (map_dir / "metadata.yaml").write_text(
        yaml.safe_dump(
            {
                "created_at": "2026-05-09T11:04:44+08:00",
                "representation": "pointcloud_map_3d",
                "artifacts": [{"kind": "pointcloud_map_3d", "path": "pointcloud_map_3d.pcd"}],
            },
            sort_keys=False,
            allow_unicode=True,
        ),
    ...
    [saved_map] = controller.list_maps()

    assert saved_map.map_id == "dlio_21534_map"
    assert saved_map.has_pointcloud_3d is True
    assert saved_map.navigation_compatible is True
