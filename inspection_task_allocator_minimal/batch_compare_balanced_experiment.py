...
def run_method(allocator_class, grid_map, tasks, start_pos, balanced=False):
    tasks_copy = copy.deepcopy(tasks)
    if balanced and allocator_class is PriorityCostTaskAllocator:
...
def main():
    ...
    results_path = results_dir / "batch_compare_balanced_results.csv"
    summary_path = results_dir / "batch_compare_balanced_summary.csv"

    methods = [
        ("FS", FixedSequenceAllocator),
        ("NNF", NearestNeighborAllocator),
        ("AStarOnly", AStarOnlyAllocator),
        ("Proposed-Balanced", PriorityCostTaskAllocator),
    ]
    ...
    for seed in seeds:
        ...
        for method_name, allocator_class in methods:
            result = run_method(allocator_class, grid_map, tasks, start_pos, balanced=(method_name == "Proposed-Balanced"))
            rows.append(
                {
                    "seed": seed,
                    "method": method_name,
