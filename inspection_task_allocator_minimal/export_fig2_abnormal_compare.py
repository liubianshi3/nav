...
def build_metrics(rows):
    ...
    return {
        "abnormal_priority_rate_mean": [
            float(row_map[method]["abnormal_priority_rate_mean"]) for method in METHOD_ORDER
        ],
        "abnormal_avg_response_time_mean": [
            float(row_map[method]["abnormal_avg_response_time_mean"]) for method in METHOD_ORDER
        ],
        "high_priority_avg_response_time_mean": [
            float(row_map[method]["high_priority_avg_response_time_mean"]) for method in METHOD_ORDER
        ],
        "total_inspection_time_mean": [
            float(row_map[method]["total_inspection_time_mean"]) for method in METHOD_ORDER
        ],
    }
...
def plot(csv_path, output_path):
    ...
    specs = [
        ("（a）异常任务优先处理率", "比例 / %", metrics["abnormal_priority_rate_mean"], "#4C78A8"),
        ("（b）异常平均响应时间", "时间 / s", metrics["abnormal_avg_response_time_mean"], "#F58518"),
        ("（c）高优先级任务平均响应时间", "时间 / s", metrics["high_priority_avg_response_time_mean"], "#54A24B"),
        ("（d）总巡检时间", "时间 / s", metrics["total_inspection_time_mean"], "#E45756"),
    ]
    ...
    fig.suptitle("异常反馈实验结果对比", fontsize=16)
...
def main():
    ...
    csv_path = base_dir / "results" / "rh_v2_abnormal_summary.csv"
    output_path = base_dir / "results" / "figures" / "fig2_abnormal_compare.png"
