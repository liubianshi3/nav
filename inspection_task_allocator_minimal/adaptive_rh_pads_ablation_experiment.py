...
METHODS = [
    "A-RH-PADS-Full",
    "A-RH-PADS-FixedLambda",
    "A-RH-PADS-NoUrgencyPressure",
    "A-RH-PADS-NoAbnormalPressure",
    "A-RH-PADS-NoPathPressure",
    "A-RH-PADS-ResponseOnly",
    "A-RH-PADS-CostOnly",
    "A-RH-PADS-NoFinishTimeResponse",
]
...
def main():
    ...
    results_path = results_dir / "adaptive_rh_pads_ablation_results.csv"
    summary_path = results_dir / "adaptive_rh_pads_ablation_summary.csv"
    ...
    print_summary_table(summary_rows, CHANGE_METRICS + ["lambda_mean", "lambda_std"])
    summary_map = {row["method"]: row for row in summary_rows}
    for base_method in METHODS[1:]:
        print_change_block(summary_map, "A-RH-PADS-Full", base_method, CHANGE_METRICS)
