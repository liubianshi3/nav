...
def print_change_block(summary_map, target_method, base_method):
    ...
    print(
        f"- total_path_length: {percent_change(target['total_path_length_mean'], base['total_path_length_mean']):.2f}%"
    )
    print(
        f"- total_inspection_time: {percent_change(target['total_inspection_time_mean'], base['total_inspection_time_mean']):.2f}%"
    )
    print(
        f"- high_priority_avg_response_time: {percent_change(target['high_priority_avg_response_time_mean'], base['high_priority_avg_response_time_mean']):.2f}%"
    )
    print(
        f"- priority_weighted_completion_time: {percent_change(target['priority_weighted_completion_time_mean'], base['priority_weighted_completion_time_mean']):.2f}%"
    )
    print(
        f"- high_priority_top5_rate: {percent_change(target['high_priority_top5_rate_mean'], base['high_priority_top5_rate_mean']):.2f}%"
    )
...
def main():
    ...
    results_path = results_dir / "rh_v2_compare_results.csv"
    summary_path = results_dir / "rh_v2_compare_summary.csv"
    ...
    print_change_block(summary_map, "RH-Proposed-v2", "Proposed-Balanced")
    print_change_block(summary_map, "RH-Proposed-v2", "RH-Proposed-v1")
    print_change_block(summary_map, "RH-Proposed-v2", "AStarOnly")

    print(
        "\n说明：路径、时间、响应时间、加权完成时间越小越好；top5 rate 越大越好。"
    )
