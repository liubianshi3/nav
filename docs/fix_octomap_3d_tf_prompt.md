# OBSOLETE: historical diagnosis only

This note is kept for history. The Unitree A2 SDK structural lidar2baselink matrix is:
...
Do not use this document as justification for `0.0, -1.0, 0.0`.
...
2. **`octomap_mapping.launch.py` 中 `lidar_to_base_rotation` 第三行符号错误**
   ...
   - 历史误判值: `[0.0, -1.0, 0.0]`
   - 该结论已废弃；当前应以 Unitree A2 SDK 结构外参为准：
...
1. 历史修正方向已废弃：不要再将 `0.0, 1.0, 0.0` 改为 `0.0, -1.0, 0.0`
