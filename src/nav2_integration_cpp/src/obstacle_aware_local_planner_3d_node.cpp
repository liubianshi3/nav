...
private:
  void declare_and_load_params()
  {
    ...
    self_filter_radius_ = declare_parameter<double>("self_filter_radius", 0.35);
  ...
  void on_obstacle_cloud(sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
  {
    ...
    try {
      ...
      for (std::size_t i = 0; i < n; ++i, ++ix, ++iy, ++iz) {
        ...
        if (distance_sq < self_filter_radius_ * self_filter_radius_) {
          continue;
        }
        if (distance_sq >
  ...
  void publish_status_locked(bool ready, const std::string & state, const std::string & reason)
  {
    ...
    msg.data =
      ...
      ";self_filter_radius=" + std::to_string(self_filter_radius_) +
      ";planner=dwa_lite";
