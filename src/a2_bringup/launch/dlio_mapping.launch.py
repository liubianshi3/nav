...
def _launch_setup(context, *args, **kwargs):
    ...
    start_octomap = _as_bool(LaunchConfiguration("start_octomap").perform(context))
    ...
    if start_octomap and start_dlio:
        actions.append(
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(f"{bringup_share}/launch/octomap_mapping.launch.py"),
                launch_arguments={
                    "use_sim_time": str(use_sim_time).lower(),
                    "frame_id": "odom",
                    "odom_topic": "/jt128/dlio/odom",
                    "cloud_topic": pointcloud_topic,
                    "filtered_cloud_topic": "/a2/octomap/cloud_in",
                    "save_path": f"{map_root}/octomap_live.bt",
