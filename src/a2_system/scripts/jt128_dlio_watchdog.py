...
class Jt128DlioWatchdog(Node):
    ...
    def on_odom(self, msg: Odometry) -> None:
        ...
        planar_speed = math.sqrt(v.x * v.x + v.y * v.y)
        vertical_speed = abs(v.z)
        ...
        elif planar_speed > self.max_linear_speed:
            reason = f"planar_speed={planar_speed:.2f}>{self.max_linear_speed:.2f}"
        if reason is None:
            self.publish_status(
                "ok",
                True,
                (
                    f"position_norm={position_norm:.3f};z={p.z:.3f};"
                    f"planar_speed={planar_speed:.3f};vertical_speed={vertical_speed:.3f};"
