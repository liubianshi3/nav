...
public:
  ...
  : Node("a2_control_bridge")
  {
    ...
    linear_x_sign_ = declare_parameter<double>("linear_x_sign", 1.0);
    linear_y_sign_ = declare_parameter<double>("linear_y_sign", 1.0);
    ...
    if (runtime_mode_ == "real") {
      if (resolved_interface_.empty()) {
        ...
        try {
          ...
          RCLCPP_WARN(get_logger(), "ChannelFactory::Init failed (possibly already initialized): %s", e.what());
        }
...
private:
  ...
  void control_tick()
  {
    ...
    if (!timed_out && gate_open) {
      ...
      limited.linear.x *= sign(linear_x_sign_);
      limited.linear.y *= sign(linear_y_sign_);
      limited.angular.z *= sign(yaw_sign_);
    }
